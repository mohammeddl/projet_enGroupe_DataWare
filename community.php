                ...
                <form action="community.php" method="get">
                  ...
                  <button type='submit' name='filterproj' value='$id' class='flex bg-[#1E293B] w-[80%] hover:ml-4 text-white hover:text-purple-500 dark:hover:text-blue-500 bg-[#1E293B] p-2 pl-8 rounded-full transform ease-in-out duration-300' filterpro>$projetnom</button>
                        ...
                        if (isset($_GET['filterproj'])) {
                            $id = $_GET['filterproj'];
                            echo "$projetnom and $id";
                        }
                        ?>
                        <script>
                            const filterbutton = document.querySelectorAll('.filterpro');
                            filterbutton.forEach(button => {
                                button.addEventListener("click", dispalayQA);

                            });

                            function dispalayQA() {
                                var xhr = new XMLHttpRequest();
                                xhr.open('GET', "config.php?projid=<?php echo $id;?>",true);
                                xhr.onload = function(){
